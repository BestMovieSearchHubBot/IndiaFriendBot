...
const registrationStage = new Scenes.WizardScene(
  'registration',
  async (ctx) => {
    await ctx.reply('Welcome to *India Friend*! 🇮🇳\nLet\'s create your profile.\n\nWhat is your name?', { parse_mode: 'Markdown' });
    return ctx.wizard.next();
  },
  async (ctx) => {
    const name = ctx.message.text.trim();
    if (name.length < 2) {
      await ctx.reply('Name must be at least 2 characters. Please enter again.');
      return;
    }
    ctx.wizard.state.name = name;
    await ctx.reply('What is your age? (only number)');
    return ctx.wizard.next();
  },
  async (ctx) => {
    const age = parseInt(ctx.message.text);
    if (isNaN(age) || age < 18) {
      await ctx.reply('Please enter a valid age (must be 18 or older).');
      return;
    }
  ...
  async (ctx) => {
    if (!ctx.callbackQuery) {
      await ctx.reply('Please use the buttons to select your gender.');
      return;
    }
    ...
    await ctx.reply('Select your preferred language:', langKeyboard);
    return ctx.wizard.next();
  },
  async (ctx) => {
    if (!ctx.callbackQuery) {
      await ctx.reply('Please use the buttons to select your language.');
      return;
    }
    ...
    await ctx.answerCbQuery();
    await ctx.reply('Now send me a photo of yourself.');
    return ctx.wizard.next();
  },
  async (ctx) => {
...
bot.command('find', async (ctx) => {
  ...
  if (user.active_match) {
    await ctx.reply('You already have an active match. Use /message to chat or /unmatch to end it.');
    return;
  }
